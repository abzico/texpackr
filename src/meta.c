...
texpackr_sheetmeta* texpackr_parse(const char* sheet_meta_filename)
{
	...
	for (int i=0; i<sprite_count; ++i)
	{
		...
		if (sscanf(line, "%s %d %d %d %d", sprite_filename, &read_offset.x, &read_offset.y, &read_size.x, &read_size.y) == 5)
		{
			...
			TEXPACKR_LOG("[META] found sprite %s offset: %d,%d size: %d,%d\n", sprite->filename, sprite->offset.x, sprite->offset.y, sprite->size.x, sprite->size.y)
		}
		else
		{
